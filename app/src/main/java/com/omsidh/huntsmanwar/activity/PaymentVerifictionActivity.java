...
public class PaymentVerifictionActivity extends AppCompatActivity {
    ...
    private EditText transactionEt,amountEt,noteEt,walletEt;
    ...
    private String transactionSt, amountSt, noteSt, walletSt;
    private int intAmount, intCoins;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        submitBt.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                amountSt = amountEt.getText().toString().trim();
                transactionSt = transactionEt.getText().toString().trim();
                noteSt = noteEt.getText().toString().trim();
                walletSt = walletEt.getText().toString().trim();
                ...
                if (is_active.equals("0")) {
                ...
                else if (!amountSt.isEmpty()) {
                    intAmount = Integer.parseInt(amountSt);
                    if (intAmount < intCoins) {
                        submitBt.setEnabled(true);
                        remarkTv.setVisibility(View.VISIBLE);
                        remarkTv.setTextColor(Color.parseColor("#ff0000"));
                    } else if (intAmount >= intCoins) {
                        if (!transactionSt.isEmpty() || !transactionSt.contains(" ")){
                            ...
                            submitBt.setEnabled(true);
                            remarkTv.setVisibility(View.VISIBLE);
                            remarkTv.setText("Please Enter Transaction Number");
                            remarkTv.setTextColor(Color.parseColor("#ff0000"));
                        }
                    }
                } else if (amountSt.isEmpty()) {
                    submitBt.setEnabled(true);
                    remarkTv.setVisibility(View.VISIBLE);
                    remarkTv.setText("Minimum amount is 20");
                    remarkTv.setTextColor(Color.parseColor("#ff0000"));
                }
            }
        });
    ...
    private void initBundle() {
        ...
        if (extras != null) {
            ...
            accountNameTv.setText(title);
            accountIdTv.setText(message);
            amountEt.setText(amount+" "+currency);
            walletEt.setText(coins);
            remarkTv.setText(subtitle);
...
//                        parameters.put("mobile", mobileNumber);
    ...
    private void initView() {
        ...
        walletEt = (EditText) findViewById(R.id.walletEt);
    ...
    private void addMoney() {
        if (new ExtraOperations().haveNetworkConnection(getApplicationContext())) {
            Uri.Builder builder = Uri.parse(Constant.ADD_MONEY_URL).buildUpon();
            builder.appendQueryParameter("access_key", Config.PURCHASE_CODE);
            builder.appendQueryParameter("user_id",id);
            builder.appendQueryParameter("transaction_id",transactionSt);
            builder.appendQueryParameter("amount",amountSt);
            builder.appendQueryParameter("coin",amountSt);
            builder.appendQueryParameter("method",walletSt);
            builder.appendQueryParameter("note",noteSt);
                ...
                public void onResponse(String response) {
                    try {
                        ...
                        if (success.equals("0")) {
                            ...
                            submitBt.setEnabled(true);
                            Toast.makeText(getApplicationContext(),msg+"", Toast.LENGTH_LONG).show();
                            Intent intent = new Intent(PaymentVerifictionActivity.this,MainActivity.class);
